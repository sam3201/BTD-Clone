...
void draw_ballon(Game *game, Ballon *ballon, Color color) {
    if (ballon->state == ALIVE) {
        DrawCircleV(ballon->pos, BALLON_RADIUS, color);
    }
}

Ballon *init_ballon(unsigned int id, Vector2 position, int target_node, Color color) {
    ...
    switch (game->curr_state) {
        ...
        case IN_PLAY:
            ...
            if (game->ballon_spawn_timer >= BALLOON_SPAWN_TIME) {
                ...
                game->ballon_count++;
                Ballon *ballon = init_ballon(game->ballon_count, game->nodes[0].pos, 1, RED); 
            ...
            for (unsigned int i = 0; i < game->ballon_count; i++) {
                ...
                draw_ballon(game, &game->ballons[i], RED);
            }
            ...
            for (unsigned int i = 0; i < game->monkey_count; i++) {
                ...
                for (unsigned int j = 0; j < game->ballon_count; j++) {
                    ...
                    if (ballon->state == ALIVE) {
                        ...
                        if (distance <= 200  && monkey->shoot_timer >= MONKEY_SHOOT_TIME) {  
